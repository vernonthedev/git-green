...
import fs from 'fs'
import { ConventionalCommitGenerator } from '../../../lib/lib/conventional-commits'
import { FormData, CommitData } from '../../../lib/types/index'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const commitMessages = commitGenerator.generateWorkflowCommits(totalCommits)

    let commitsMade = 0

    for (let i = 0; i < totalCommits; i++) {
      const randomDayOffset = Math.floor(Math.random() * (end.diff(start, 'days') + 1))
    ...
    return NextResponse.json({ error: 'Failed to generate commits' }, { status: 500 })
  }
}