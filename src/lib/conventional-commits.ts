...
export class ConventionalCommitGenerator {
  ...
  private getRandomScope(): string {
    return (
      this.scopes[Math.floor(Math.random() * this.scopes.length)] || "core"
    );
  }
  ...
  private generateBody(): string {
    ...
    return (
      ...
      "Updated implementation for better performance."
    );
  }
  ...
  generateWorkflowCommits(count: number = 10): string[] {
    ...
    for (let i = 0; i < count; i++) {
      const generator =
        commitGenerators[Math.floor(Math.random() * commitGenerators.length)];
      if (generator) {
